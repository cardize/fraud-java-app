 ...
 * fraud.scenario.max-parallelism. destroyMethod=shutdown closes it cleanly on application shutdown.
 */
@Configuration
public class ScenarioExecutorConfig {
    ...
    public ExecutorService scenarioExecutor(
            @Value("${fraud.scenario.max-parallelism:10}") int maxParallelism) {
